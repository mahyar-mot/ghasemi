...
// import {FaTwitter, FaInstagram, FaTelegram, FaRetweet, FaQuoteRight, FaReply, FaComment} from "react-icons/fa";
...
// const Icon = (props) => (
//     <small className="mx-1"> {props.detail} {React.createElement(props.logo, {style: {width: "20px", ...props.misc}})} </small>
// )


// const convertSM = (social_media) => {
//     const SM = {ig: "اینستاگرام", tg: "تلگرام", tw: "توییتر", fb: "فیسبوک", yt: "یوتیوب", news: "اخبار", newspaper: "روزنامه"}
...
//     return (
//     <div className="d-flex justify-content-between my-4 ml-4 ">
//         <div className="font-gray-color mr-3"></div>
//         <div className="d-flex justify-content-around border carousel-control" style={{width: "150px"}}> 
//             <button className="btn main-btn-round-reverse mt-1 bg-white font-weight-bold"
//                 onClick={() => props.changePage(props.prevPage)} disabled={props.prevPage === 0 && props.currentPage !== 2} >{"<"}</button>
//             <span className="mt-1"> صفحه {props.currentPage} </span>
//             <button className="btn main-btn-round-reverse mt-1 bg-white font-weight-bold"
//                 onClick={() => props.changePage(props.nextPage)} disabled={!Boolean(props.nextPage)} >{">"}</button>
//         </div>
//     </div>
// )};
...
// export const Loading = () => ( <div className="d-flex justify-content-center align-items-center" style={{minHeight: "150px"}}>
...
//                     {name}
//                     <span className="mr-2 font-size-m pointer hover-red" onClick={ e => props.deleteParam(props.item)}>&times;</span>
//                 </span>
//             ) : []
// }
...
//     }
//     return (
//         <span className={`badge badge-pill text-white px-3 py-2 ml-2 my-1 ${props.className}`} onClick={props?.onClick} style={{backgroundColor: borderColor}}>
//             {props.username}
...
//     const config = [{index: 0, color: "default", title: "خنثی"}, {index: 1, color: "green", title: "مثبت"}, {index: 2, color: "red", title: "منفی"}];

//     const [activeTag, setActiveTag] = React.useState(props.sentiment);

//     let instance = config.filter( item => item.title === activeTag )[0];

//     const cycle = (index) => { let temp = config[++index%config.length].title; setActiveTag(state => temp); props.setSentimentStatus( state => temp); };

//     return <Tag ref={ref} onClick={ () => props.editable ? cycle(instance.index) : null } className="float-left pointer ml-2" color={instance.color}>{instance.title}</Tag>
// })
